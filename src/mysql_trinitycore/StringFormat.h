...
namespace Trinity
{
    ...
    inline std::string DateFormat(std::chrono::system_clock::time_point tp)
    {
    ...
    inline std::string StringFormat(FormatString<Args...> fmt, Args&&... args)
    {
        ...
        {
            fmt::basic_string_view<char> fmt_view = fmt;
            return fmt::format("An error occurred formatting string \"{}\" : {}", fmt_view, formatError.what());
        }
    }

    template <typename OutputIt, typename... Args>
    inline OutputIt StringFormatTo(OutputIt out, fmt::format_string<Args...> fmt, Args&&... args)
    {
        try
        {
        ...
        {
            fmt::basic_string_view<char> fmt_view = fmt;
            return fmt::format_to(out, "An error occurred formatting string \"{}\" : {}", fmt_view, formatError.what());
        }
    }
