...
MySQLConnection::MySQLConnection(MySQLConnectionInfo& connInfo) :
    m_reconnecting(false), m_prepareError(false), m_queue(nullptr), m_Mysql(nullptr), m_connectionInfo(connInfo),
    m_connectionFlags(CONNECTION_SYNCH)
{
}

MySQLConnection::MySQLConnection(ProducerConsumerQueue<SQLOperation*>* queue, MySQLConnectionInfo& connInfo) :
    m_reconnecting(false), m_prepareError(false), m_queue(queue), m_Mysql(nullptr), m_connectionInfo(connInfo),
...
//mysql_options(mysqlInit, MYSQL_OPT_READ_TIMEOUT, (char const*)&timeout);
    ...
    m_Mysql = reinterpret_cast<MySQLHandle*>(mysql_real_connect(
            mysqlInit, m_connectionInfo.host.c_str(), m_connectionInfo.user.c_str(), m_connectionInfo.password.c_str(),
    ...
    if (m_Mysql)
    {
        if (!m_reconnecting)
        {
            ...
            if (mysql_get_server_version(m_Mysql) != mysql_get_client_version())
                TC_LOG_INFO("sql.sql",
                            "[WARNING] MySQL client/server version mismatch; may conflict with behaviour of prepared "
                            "statements.");
        }
    ...
    {
        TC_LOG_ERROR("sql.sql", "Could not connect to MySQL database at {}: {}", m_connectionInfo.host,
...
bool MySQLConnection::Execute(PreparedStatementBase* stmt)
{
    ...
    {
        uint32 lErrno = mysql_errno(m_Mysql);
        TC_LOG_ERROR("sql.sql", "SQL(p): {}\n [ERROR]: [{}] {}", m_mStmt->getQueryString(), lErrno,
...
bool MySQLConnection::_Query(PreparedStatementBase* stmt, MySQLPreparedStatement** mysqlStmt, MySQLResult** pResult,
                             uint64* pRowCount, uint32* pFieldCount)
{
    if (!m_Mysql)
    {
        TC_LOG_ERROR("sql.sql", "_Query: m_Mysql is null");
        return false;
    }


    uint32 index = stmt->GetIndex();
    TC_LOG_INFO("sql.sql", "_Query: index={}, m_stmts.size()={}", index, m_stmts.size());

    MySQLPreparedStatement* m_mStmt = GetPreparedStatement(index);
    if (!m_mStmt)
    {
        TC_LOG_ERROR("sql.sql", "_Query: GetPreparedStatement returned nullptr for index={}", index);
        return false;
    }
    ASSERT(m_mStmt); // Can only be null if preparation failed, server side error or bad query
    ...
    TC_LOG_INFO("sql.sql", "_Query: Got prepared statement, query string: {}", m_mStmt->getQueryString());
    ...
    *mysqlStmt = m_mStmt;
    ...
    if (!msql_STMT)
    {
        TC_LOG_ERROR("sql.sql", "_Query: msql_STMT is null");
        return false;
    }
    MYSQL_BIND* msql_BIND = m_mStmt->GetBind();
...
bool MySQLConnection::_Query(const char* sql, MySQLResult** pResult, MySQLField** pFields, uint64* pRowCount,
                             uint32* pFieldCount)
{
    if (!m_Mysql)
        return false;
...
MySQLPreparedStatement* MySQLConnection::GetPreparedStatement(uint32 index)
{
    ASSERT(index < m_stmts.size(),
           "Tried to access invalid prepared statement index %u (max index " SZFMTD
           ") on database `%s`, connection type: %s",
           index, m_stmts.size(), m_connectionInfo.database.c_str(),
           (m_connectionFlags & CONNECTION_ASYNC) ? "asynchronous" : "synchronous");
    MySQLPreparedStatement* ret = m_stmts[index].get();
    if (!ret)
        TC_LOG_ERROR("sql.sql", "Could not fetch prepared statement {} on database `{}`, connection type: {}.", index,
    ...
    switch (errNo)
    {
        ...
        case CR_CONN_HOST_ERROR:
            {
                ...
                if (!lErrno)
                {
                    ...
                    TC_LOG_INFO("sql.sql", "Successfully reconnected to {} @{}:{} ({}).", m_connectionInfo.database,
                                m_connectionInfo.host, m_connectionInfo.port_or_socket,
                                (m_connectionFlags & CONNECTION_ASYNC) ? "asynchronous" : "synchronous");
        ...
        case ER_NO_SUCH_TABLE:
            TC_LOG_ERROR("sql.sql",
                         "Your database structure is not up to date. Please make sure you've executed all queries in "
                         "the sql/updates folders.");
            std::this_thread::sleep_for(std::chrono::seconds(10));
            ABORT();
            return false;
        case ER_PARSE_ERROR:
