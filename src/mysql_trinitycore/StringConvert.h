...
namespace Trinity::Impl::StringConvertImpl
{
    ...
    struct For<T, std::enable_if_t<std::is_integral_v<T> && !std::is_same_v<T, bool>>>
    {
        ...
        static std::string ToString(T val)
        {
            ...
            std::string                buf(buffer_size::value,
                                           '\0'); /* 2^64 is 20 decimal characters, -(2^63) is 20 including the sign */
    ...
    */
    ...
    struct For<bool, void>
    {
        static Optional<bool> FromString(
                std::string_view str,
                int              strict = 0) /* this is int to match the signature for "proper" integral types */
        {
            if (strict)
            {
                if (str == "1")
...
namespace Trinity
{
    ...
    {
        return Trinity::Impl::StringConvertImpl::For<std::decay_t<Type>>::ToString(std::forward<Type>(val),
