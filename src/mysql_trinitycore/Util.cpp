...
void Trinity::VerifyOsVersion()
{
    ...
    if (!isWindowsBuildGreaterOrEqual(TRINITY_REQUIRED_WINDOWS_BUILD))
    {
        ...
        ABORT_MSG("TrinityCore requires Windows 10 19H1 (1903) or Windows Server 2019 (1903) - require build number "
                  "10.0.%d but found %d.%d.%d",
...
std::string TimeToTimestampStr(time_t t)
{
    ...
    return Trinity::StringFormat("{:04}-{:02}-{:02}_{:02}-{:02}-{:02}", aTm.tm_year + 1900, aTm.tm_mon + 1, aTm.tm_mday,
...
std::string Trinity::Impl::ByteArrayToHexStr(uint8 const* bytes, size_t arrayLen, bool reverse /* = false */)
{
    ...
    {
        uint32 val = bytes[i];
        Trinity::StringFormatTo(inserter, "{:02X}", val);
    }
