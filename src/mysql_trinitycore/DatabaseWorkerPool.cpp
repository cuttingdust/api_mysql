...
DatabaseWorkerPool<T>::DatabaseWorkerPool() :
    _queue(new ProducerConsumerQueue<SQLOperation*>()), _async_threads(0), _synch_threads(0)
{
...
#endif
    WPFatal(mysql_get_client_version() == TRINITY_COMPILED_CLIENT_VERSION,
            "Used " TRINITY_MYSQL_FLAVOR " library version (%s id %lu) does not match the version id used to compile "
            "TrinityCore (id %u). Search on forum for TCE00011.",
            mysql_get_client_info(), mysql_get_client_version(), TRINITY_COMPILED_CLIENT_VERSION);
...
template <class T>
void DatabaseWorkerPool<T>::SetConnectionInfo(std::string const& infoString, uint8 const asyncThreads,
        ...
        for (auto& connection : connections)
        {
            ...
            for (size_t i = 0; i < preparedSize; ++i)
            {
                ...
                if (MySQLPreparedStatement* stmt = connection->m_stmts[i].get())
                {
                    ...
                    TC_LOG_INFO("sql.driver", "Statement index {} has {} parameters", i, paramCount);
...
PreparedQueryResult DatabaseWorkerPool<T>::Query(PreparedStatement<T>* stmt)
{
    ...
    if (ret)
    {
        TC_LOG_INFO("sql.sql", "PreparedQuery returned result set with {} rows", ret->GetRowCount());
    }
    else
    {
        TC_LOG_ERROR("sql.sql", "PreparedQuery returned nullptr");
    }
...
{
    TC_LOG_INFO("sql.driver", "GetPreparedStatement index={}, capacity={}", static_cast<uint32>(index),
                _preparedStatementSize[index]);
...
uint32 DatabaseWorkerPool<T>::OpenConnections(InternalIndex type, uint8 numConnections)
{
    for (uint8 i = 0; i < numConnections; ++i)
    {
        ...
        else if (uint32 serverVersion = connection->GetServerVersion();
                 serverVersion < ParseVersionString(TRINITY_REQUIRED_MYSQL_VERSION))
        {
            TC_LOG_ERROR("sql.driver",
                         "TrinityCore does not support " TRINITY_MYSQL_FLAVOR
                         " versions below " TRINITY_REQUIRED_MYSQL_VERSION
                         " (found id {}, need id >= {}), please update your " TRINITY_MYSQL_FLAVOR " server",
                         serverVersion, ParseVersionString(TRINITY_REQUIRED_MYSQL_VERSION));
            return 1;
        }
        else
