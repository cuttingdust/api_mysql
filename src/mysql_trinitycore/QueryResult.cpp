...
namespace
{
    ...
    public:
        static MYSQL_TIME GetDatabaseValue(char const* data, uint32 size)
        {
            ...
            if (!parseNextComponent(yearOrHours, firstSeparator) ||
                !parseNextComponent(monthOrMinutes, firstSeparator) || !parseNextComponent(dayOrSeconds))
            {
            ...
            else
            {
                ...
                uint32 minutes = 0;
                uint32 seconds = 0;
                if (!parseNextComponent(hours, ':') || !parseNextComponent(minutes, ':') ||
        ...
        SystemTimePoint GetDate(char const* data, uint32 size, QueryResultFieldMetadata const* meta) const override
        {
            ...
            switch (source.time_type)
            {
                ...
                case MYSQL_TIMESTAMP_DATETIME:
                    return sys_days(year(source.year) / month(source.month) / day(source.day)) + hours(source.hour) +
                            minutes(source.minute) + seconds(source.second) + microseconds(source.second_part);
                default:
                    break;
            }
    ...
    std::unique_ptr<BaseDatabaseResultValueConverter> const BinaryValueConverters[15] = {
        ...
        std::make_unique<
                PrimitiveResultValueConverter<double, FromStringToDatabaseTypeConverter>>(), // always sent as string
                                         ...
                                         bool binaryProtocol)
    {
        ...
        meta->Converter  = binaryProtocol ? BinaryValueConverters[AsUnderlyingType(meta->Type)].get()
                                          : FromStringValueConverters[AsUnderlyingType(meta->Type)].get();
    }
} // namespace
...
PreparedResultSet::PreparedResultSet(MySQLStmt* stmt, MySQLResult* result, uint64 rowCount, uint32 fieldCount) :
    m_rowCount(rowCount), m_rowPosition(0), m_fieldCount(fieldCount), m_rBind(nullptr), m_stmt(stmt),
    ...
    while (_NextRow())
    {
        for (uint32 fIndex = 0; fIndex < m_fieldCount; ++fIndex)
        {
            ...
            if (!*m_rBind[fIndex].is_null)
            {
                ...
                m_rows[std::size_t(m_rowPosition) * m_fieldCount + fIndex].SetValue((char const*)buffer,
                                                                                    fetched_length);
            ...
            {
                m_rows[std::size_t(m_rowPosition) * m_fieldCount + fIndex].SetValue(nullptr, *m_rBind[fIndex].length);
            }
        }
        m_rowPosition++;
    }
