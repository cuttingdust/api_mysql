...
# 获取当前CMakeLists.txt所在目录下的所有子目录（递归）
macro(get_all_subdirectories result_var)
    # 清除结果变量
    set(${result_var} "")
    
    # 递归获取所有文件和目录
    file(GLOB_RECURSE ALL_ITEMS 
    ...
    foreach(item ${ALL_ITEMS})
        ...
        if(IS_DIRECTORY ${item})
            # 获取相对于当前目录的路径
            file(RELATIVE_PATH rel_path ${CMAKE_CURRENT_LIST_DIR} ${item})
            # 添加到结果列表
            list(APPEND ${result_var} ${rel_path})
        endif()
    endforeach()
    
    # 排序并去重（可选）
    list(SORT ${result_var})
    list(REMOVE_DUPLICATES ${result_var})
    
    # 打印结果（用于调试）
    # message("Found subdirectories: ${${result_var}}")
endmacro()

# 使用示例
get_all_subdirectories(ALL_SUBDIRS)
message("All subdirectories: ${ALL_SUBDIRS}")
...
macro(get_src_include)
	...
	###################################################################
	get_all_subdirectories(ALL_SUBDIRS)
	message(STATUS "All subdirectories: ${ALL_SUBDIRS}")
	
	# 处理每个子目录
    foreach(subdir ${ALL_SUBDIRS})
		# 构建完整路径
        set(full_subdir_path "${CMAKE_CURRENT_LIST_DIR}/${subdir}")
		message(STATUS "full_subdir_path = ${full_subdir_path}")
        
        # 获取该子目录下的源文件
        file(GLOB SUBDIR_SRC 
        ...
        file(GLOB SUBDIR_H 
            "${full_subdir_path}/*.h"
            "${full_subdir_path}/*.hpp"
        )
        
		# 创建 subdir_list 变量来存储此子目录的所有文件
		set(SUBDIR_LIST ${SUBDIR_SRC} ${SUBDIR_H})
      
        if(SUBDIR_LIST)
            source_group("${subdir}" FILES ${SUBDIR_LIST})
        endif()
    endforeach() 	
	##################################################################
	FILE(GLOB ASM_FILES 
